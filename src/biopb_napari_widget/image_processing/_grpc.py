...
def _check_chunk_memory(chunk) -> None:
    ...
    """
    warn_mb = SETTINGS.get("memory.warn_threshold_mb")
    error_mb = SETTINGS.get("memory.error_threshold_mb")
...
def _get_grpc_channel(settings: dict):
    ...
    max_msg_size = SETTINGS.get("grpc.max_message_size_mb")
...
def check_server_health(settings: dict, timeout: float | None = None) -> bool:
    ...
    """
    if timeout is None:
        timeout = SETTINGS.get("timeout.health_check")
...
def get_op_names(settings: dict, timeout: float | None = None) -> proto.OpNames:
    ...
    """
    if timeout is None:
        timeout = SETTINGS.get("timeout.get_op_names")
