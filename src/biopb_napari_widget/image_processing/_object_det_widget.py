...
from .._settings import SETTINGS, get_grid_params
...
class ObjectDetectionWidget(_WidgetBase):
    ...
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self._threshold = create_widget(
            value=SETTINGS.get("detection.min_score"),
            label="Min Score",
            annotation=float,
        ...
        self._size_hint = create_widget(
            value=SETTINGS.get("detection.size_hint"),
            label="Size Hint",
            annotation=float,
            widget_type="FloatSlider",
        ...
        self._nms = ComboBox(
            value=SETTINGS.get("detection.nms"),
        ...
        self._aspect_ratio = create_widget(
            value=SETTINGS.get("detection.z_aspect_ratio"),
    ...
    def _get_grid_positions(self, image, settings: dict) -> List[Tuple[slice, ...]]:
        ...
        """
        is_3d = settings["3D"]
        grid_size, stride = get_grid_params(is_3d)
    ...
    def _save_config(self):
        """Save this widget's fields to the plugin settings in one write."""
        settings = self._snapshot()
        SETTINGS.set("widget.server_url", settings["Server"], persist=False)
        SETTINGS.set("widget.is_3d", settings["3D"], persist=False)
        SETTINGS.set("detection.min_score", settings["Min Score"], persist=False)
        SETTINGS.set("detection.size_hint", settings["Size Hint"], persist=False)
        SETTINGS.set("detection.nms", settings["NMS"], persist=False)
        SETTINGS.set(
